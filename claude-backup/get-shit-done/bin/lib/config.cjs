...
function setConfigValue(cwd, keyPath, parsedValue) {
  ...
  // Reject __proto__/constructor/prototype segments before walking the path --
  // this function assigns through `current[key] = {}` on each segment, which
  // would otherwise let a caller passing an unvalidated keyPath (this is a
  // shared utility, not just the config-set CLI command that already checks
  // against VALID_CONFIG_KEYS) pollute Object.prototype.
  const DANGEROUS_KEYS = new Set(['__proto__', 'constructor', 'prototype']);
  if (keys.some(key => DANGEROUS_KEYS.has(key))) {
    error(`Invalid config key: "${keyPath}" contains a reserved property name`);
  }
