...
function cmdPhaseAdd(cwd, description, raw, customId) {
  ...
  if (customId || config.phase_naming === 'custom') {
    ...
    newPhaseId = customId || slug.toUpperCase();
...
function updateRoadmapAfterPhaseRemoval(roadmapPath, targetPhase, isDecimal, removedInt) {
  ...
  if (!isDecimal) {
    ...
    for (let oldNum = MAX_PHASE; oldNum > removedInt; oldNum--) {
      ...
      // oldStr/oldPad are always plain digits here (String() of a loop counter),
      // but escape them anyway before building a RegExp from them -- same
      // discipline as escapeRegex() calls elsewhere in this file, and avoids
      // treating a numeric-looking value as pre-sanitized for regex purposes.
      const oldStrEsc = escapeRegex(oldStr);
      const oldPadEsc = escapeRegex(oldPad);
      content = content.replace(new RegExp(`(#{2,4}\\s*Phase\\s+)${oldStrEsc}(\\s*:)`, 'gi'), `$1${newStr}$2`);
      content = content.replace(new RegExp(`(Phase\\s+)${oldStrEsc}([:\\s])`, 'g'), `$1${newStr}$2`);
      content = content.replace(new RegExp(`${oldPadEsc}-(\\d{2})`, 'g'), `${newPad}-$1`);
      content = content.replace(new RegExp(`(\\|\\s*)${oldStrEsc}\\.\\s`, 'g'), `$1${newStr}. `);
      content = content.replace(new RegExp(`(Depends on:\\*\\*\\s*Phase\\s+)${oldStrEsc}\\b`, 'gi'), `$1${newStr}`);
    }
  }
