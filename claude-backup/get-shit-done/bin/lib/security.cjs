...
function sanitizeForPrompt(text) {
  ...
  // Note: <instructions> is excluded — GSD uses it as legitimate prompt structure
  // Tolerates attributes/whitespace before the closing '>' (e.g. "<system >",
  // "<system foo="bar">") -- the previous pattern required '>' immediately
  // after the tag name, so any of those variants passed through unneutralized
  // while still being interpreted as a system-boundary tag by an LLM reader.
  sanitized = sanitized.replace(/<(\/?)(?:system|assistant|human)(?:\s[^<>]*)?>/gi,
