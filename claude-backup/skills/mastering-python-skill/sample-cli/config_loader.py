...
def _partial_reveal(secret: "SecretStr | None") -> str | None:
    """Reveal just enough of a secret to confirm which one is loaded.

    Shows the last 4 characters only (same convention as how card numbers
    or API key fingerprints are typically displayed) -- never the full
    value, even when the caller explicitly asked to see secrets.
    """
    if not secret:
        return None
...
class AppSettings(BaseSettings):
    ...
    def to_safe_dict(self, show_secrets: bool = False) -> dict[str, Any]:
        """Convert to dict, optionally revealing a partial secret value.

        Even with show_secrets=True, this never prints a secret's full value --
        only enough of the tail to let you confirm which secret is loaded
        (e.g. distinguishing a dev key from a prod key). Full clear-text
        reveal would defeat the point of using SecretStr in the first place,
        and a config dump like this is exactly the kind of output that ends
        up captured in a terminal scrollback, CI log, or bug report.
        """
        data = self.model_dump()

        data["secret_key"] = _partial_reveal(self.secret_key) if show_secrets else (
            "***" if self.secret_key else None
        )
        data["api_key"] = _partial_reveal(self.api_key) if show_secrets else (
            "***" if self.api_key else None
        )
        if data["redis"]["password"]:
            data["redis"]["password"] = (
                _partial_reveal(self.redis.password) if show_secrets else "***"
            )
